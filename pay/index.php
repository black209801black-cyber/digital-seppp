...
if(!isset($_GET['zarinpal']) && !isset($_GET['nowpayment']) && !isset($_GET['nextpay']) && !isset($_GET['woocommerce'])){ // <--- اینجا ووکامرس اضافه شد
...
if(mysqli_num_rows($payInfo)==0){
    ...
    elseif(isset($_GET['zarinpal'])){
        ...
        $stmt->close();
        $first_name = isset($userinfo['name']) ? $userinfo['name'] : '';
        // --- ساخت توضیحات جدید ---
        $description = "خرید به شماره سفارش $orderId | خریدار: $first_name";
    ...
    elseif(isset($_GET['nextpay'])){        
        ...
        // بررسی فعال بودن درگاه ووکامرس در تنظیمات ربات
        if ($botState['woocommerceGatewayState'] !== 'on') {
            showForm("درگاه پرداخت در حال حاضر غیرفعال است.");
            exit();
        }
        ...
        $cell_number = $userInfo['phone'];
        
        // آدرس سایت پراکسی (thunderfile.ir) و دامنه ربات (powerspeedbot.sbs) از تنظیمات خوانده شود
        $proxy_url = !empty($botState['proxy_url']) ? $botState['proxy_url'] : 'https://thunderfile.ir/';
        $bot_domain = !empty($botState['bot_domain']) ? $botState['bot_domain'] : 'powerspeedbot.sbs'; // دامنه ربات خود را اینجا وارد کنید
        
        // آدرس بازگشت (Callback) به فایل back.php ربات
        $callback_url = $botUrl . 'pay/back.php?woocommerce';
        
        // ارسال درخواست به افزونه Exella Payment
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $proxy_url . 'wp-json/exella/v1/payment-proxy/');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query(array(
            'amount' => $amount * 10, // تبدیل به ریال
            'token' => $hash_id, // از hash_id به عنوان توکن استفاده می‌کنیم
            'domain' => $bot_domain,
            'order_id' => $orderId,
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // در صورت نیاز به true تغییر دهید
        ...
        if ($error) {
            showForm('خطا در اتصال به سرور پرداخت: ' . $error);
            exit;
        }

        $body = json_decode($response, true);
        
        if (isset($body['status']) && $body['status'] === 'success' && isset($body['redirect'])) {
            // هدایت کاربر به صفحه پرداخت که توسط Exella ایجاد شده
            header('Location: ' . $body['redirect']);
            exit;
        } else {
            $error_message = isset($body['message']) ? $body['message'] : 'خطا در ایجاد لینک پرداخت';
            showForm('خطا: ' . $error_message);
            exit;
        }
    }
}
