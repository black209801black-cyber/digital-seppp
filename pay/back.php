...
// v-- این بلاک کد کامل اضافه شده است --v
elseif(isset($_GET['woocommerce'])){
    // دریافت پارامترهای بازگشتی از افزونه Exella
    $status = isset($_GET['status']) ? $_GET['status'] : '';
    $ref_id = isset($_GET['ref_id']) ? $_GET['ref_id'] : '';
    $order_id = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
    $message = isset($_GET['message']) ? urldecode($_GET['message']) : '';
    
    // بررسی وجود سفارش در دیتابیس ربات
    ...
    if ($payInfo->num_rows == 0) {
        showForm("سفارش یافت نشد یا قبلاً پردازش شده است.", "خطا!");
        exit;
    }
    
    // بررسی وضعیت پرداخت
    if ($status === 'success') {
        // پرداخت موفق بوده است، تابع doAction را فراخوانی کن
        doAction($order_id, "woocommerce_saman");
    } else {
        ...
        showForm("پرداخت شما ناموفق بود: " . $message, "خطا در پرداخت");
    }
}
// ^-- این بلاک کد کامل اضافه شده است --^
else{
...
⏰ مدت سرویس: $days روز
";
            if($botState['configLinkState'] != "off" && $serverType != "marzban") {
                $acc_text .= "
💝 config : <code>$link</code>";
            }
            if(($botState['volumeWebState'] ?? "off") == "on") {
                $acc_text .= "
...
$currentUserInfo = isset($userInfo) ? $userInfo : (isset($user_info) ? $user_info : (isset($user_detail) ? $user_detail : null));
