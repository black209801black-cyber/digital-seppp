...
if($data=="inviteSetting" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    $stmt->execute();
    // مقدار را به عنوان درصد نمایش می‌دهیم.
    ...
    $keys = json_encode(['inline_keyboard'=>[
        ...
        [
            // متن دکمه را هم آپدیت می‌کنیم
            ['text'=>$inviteAmount,'callback_data'=>"editInviteAmount"],
            ['text'=>"درصد پورسانت",'callback_data'=>"wizwizch"]
            ],
        [
...
if($data=="editInviteAmount" && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    delMessage();
    // متن پیام را تغییر می‌دهیم تا ادمین درصد وارد کند.
    sendMessage("لطفا درصد پورسانت معرفی را وارد کنید (فقط عدد، بدون علامت درصد). مثلا برای 10 درصد، عدد 10 را ارسال کنید.",$cancelKey);
    setUser($data);
} 
...
if($data=="inviteFriends"){
    ...
    if($inviteText != null){
        ...
        $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'INVITE_BANNER_AMOUNT'");
        $stmt->execute();
        $inviteAmount = number_format($stmt->get_result()->fetch_assoc()['value']??0) . " درصد";
        $stmt->close();
...
if($userInfo['step'] == "increaseMyWallet" && $text != $buttonValues['cancel']){
    ...
    if($botState['woocommerceGatewayState'] == "on") $keyboard[] = [['text' => $buttonValues['woocommerce_gateway'],  'url' => $botUrl . "pay/index.php?woocommerce&hash_id=" . $hash_id]];
...
💝 config : <code>$link</code>":"");
 ...
 if(($botState['volumeWebState'] ?? "off") == "on") $acc_text .= "
...
// این کد بعد از موفقیت‌آمیز بودن خرید کاربر جدید اجرا می‌شود
$currentUserInfo = isset($userInfo) ? $userInfo : (isset($user_info) ? $user_info : null);
if($currentUserInfo != null && $currentUserInfo['refered_by'] != null){
    $inviterId = $currentUserInfo['refered_by']; 
    
    // بررسی اینکه آیا معرف، نماینده است یا خیر
    $stmt = $connection->prepare("SELECT `is_agent` FROM `users` WHERE `userid` = ?");
    $stmt->bind_param("i", $inviterId);
    $stmt->execute();
    $inviterInfo = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    
    // فقط در صورتی که معرف، نماینده نباشد کمیسیون پرداخت می شود
    if ($inviterInfo && $inviterInfo['is_agent'] != 1) {
        // خواندن درصد پاداش از تنظیمات
        $stmt = $connection->prepare("SELECT * FROM `setting` WHERE `type` = 'INVITE_BANNER_AMOUNT'");
        $stmt->execute();
        $invitePercent = $stmt->get_result()->fetch_assoc()['value'] ?? 0; // حالا این یک درصد است
        $stmt->close();
        
        if ($invitePercent > 0) {
            $currentPrice = isset($price) ? $price : $amount;
            // محاسبه مبلغ پاداش بر اساس درصد
            $rewardAmount = floor(($currentPrice * $invitePercent) / 100);
    
            if ($rewardAmount > 0) {
                // افزایش موجودی کیف پول کاربر معرف
                $stmt = $connection->prepare("UPDATE `users` SET `wallet` = `wallet` + ? WHERE `userid` = ?");
                $stmt->bind_param("ii", $rewardAmount, $inviterId);
                $stmt->execute();
                $stmt->close();
                 
                // ارسال پیام به معرف
                sendMessage("تبریک! یکی از زیرمجموعه‌های شما خرید انجام داد و شما مبلغ " . number_format($rewardAmount) . " تومان پاداش دریافت کردید.",null,null,$inviterId);
            }
        }
    }
}
...
if((preg_match('/^discountCustomPlanDay(\d+)/',$userInfo['step'], $match) || preg_match('/enterCustomPlanName(\d+)_(\d+)_(\d+)_(\d+)_(?<buyType>\w+)/',$userInfo['step'], $match)) && ($botState['sellState']=="on" ||$from_id ==$admin) && $text != $buttonValues['cancel']){
    ...
    if($botState['woocommerceGatewayState'] == "on") $keyboard[] = [['text' => $buttonValues['woocommerce_gateway'],  'url' => $botUrl . "pay/index.php?woocommerce&hash_id=" . $hash_id]];    if($botState['zarinpal'] == "on") $keyboard[] = [['text' => $buttonValues['zarinpal_gateway'],  'url' => $botUrl . "pay/?zarinpal&hash_id=" . $hash_id]];
    if($botState['nextpay'] == "on") $keyboard[] = [['text' => $buttonValues['nextpay_gateway'],  'url' => $botUrl . "pay/?nextpay&hash_id=" . $hash_id]];
    if($botState['weSwapState'] == "on") $keyboard[] = [['text' => $buttonValues['weswap_gateway'],  'callback_data' => "payWithWeSwap" . $hash_id]];
...
if(preg_match('/^agencyApprove(\d+)_(\d+)/',$userInfo['step'],$match) && $text != $buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    if(is_numeric($text)){
        ...
        // قطع کردن لینک زیرمجموعه گیری قبلی
        $stmt = $connection->prepare("UPDATE `users` SET `refered_by` = NULL WHERE `userid` = ?");
        $stmt->bind_param("i", $match[1]);
        $stmt->execute();
        $stmt->close();
...
if(preg_match('/(addNewRahgozarPlan|addNewPlan|addNewMarzbanPlan)/',$userInfo['step']) and $text!=$buttonValues['cancel'] && ($from_id == $admin || $userInfo['isAdmin'] == true)){
    ...
    if($step==3 and $text!=$buttonValues['cancel']){
        ...
        sendMessage("لطفا یکی از سرورها رو انتخاب کن 🐀𡰠", json_encode([
